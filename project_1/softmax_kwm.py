...
def compute_z(x,W,b):
    ...
    '''
    #########################################
    z = np.matmul(np.expand_dims(x,0),W.T).T + b
    #########################################
    return z 
...
#--------------------------
# train
def train(X, Y, Xt, Yt, alpha=0.01, n_epoch=100):
    '''
    ...
    for it in range(n_epoch):
        ...
        for x,y in zip(X,Y):
            ...
            #########################################
        print('Epoch: ', '(', it + 1, '/', n_epoch, ')')
        ypred, _ = predict(Xt, W, b)
        accuracy = sum(ypred == Yt) / len(Yt)
